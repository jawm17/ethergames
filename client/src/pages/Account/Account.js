...
export default function Account() {
  ...
  const [qrCode, setQrCode] = useState(
    "http://cdn.shopify.com/s/files/1/0062/7892/products/S7004-solid-white_352be2c2-3ecc-4181-8e8c-d75864efbd23.jpg?v=1537978455"
  );
          ...
          if (numTx < blockData.result.length) {
            ...
            UserService.updateNumTx(blockData.result.length).then((data) => {
              ...
              for (var i = blockData.result.length - 1; i >= numTx; i--) {
                if (
                  blockData.result[i].to.toUpperCase() === address.toUpperCase()
                ) {
                  console.log(
                    "reciceved: " +
      ...
      if (!message) {
        ...
        TxHistoryService.getBlockTx(data.address).then((data2) => {
          if (data2) {
            setTxs(
              data.recievedTx
                .concat(data.sentTx.concat(data2.result))
                .sort(
                  (a, b) =>
                    a.timeStamp.toString().substring(0, 9) -
                    b.timeStamp.toString().substring(0, 9)
    ...
    <div id="accountArea">
      ...
      <section className="containerAccount">
        <div className="left-container">
          ...
          <div className="balance">Balance: {parseFloat(balance)} ETH</div>
        </div>
                  ...
                  {txs.map((tx) => {
                    return (
                      <WalletTx
                        amount={
                          parseFloat(
                            (tx.value / 1000000000000000000).toFixed(6)
                          ) || tx.amount
                        }
                        address={address}
                        from={tx.from}
